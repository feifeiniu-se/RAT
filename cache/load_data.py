...
def read_commits(path):
    ...
    # cursor.execute("select commit_hash, committed_date, message, file_path from v_commit_change_file where change_type='MODIFY' and file_path like '%.java%'") #todo
    ...
    for tmp in result:
        ...
        if "merge pull request" in tmp[2].lower(): # exclude merge files
            continue
        # if "fix" in tmp[2].lower() or "bug" in tmp[2].lower() or fileName in tmp[2].lower():
...
def insert_database_vector(path, bugs):
    data = []
    for bug in bugs:
        if len(bug.cache_score_new)>0:
            for f in bug.cache_score_new:
                x = [bug.issue_id, f, str(bug.cache_score_new[f])]
                data.append(x)
    ...
    cursor.execute("create table Cache_new (issue_id text, file_path text, vector text)")
    cursor.executemany("insert into Cache_new values(?,?,?)", data)
    connection.commit()
    cursor.close()
    connection.close()
